"""Strategy: single-table GROUP BY with HAVING clause."""
...
@st.composite
def single_table_having(draw):
    """Scenario: SELECT <grp>, <agg>(<col>) FROM <table> GROUP BY <grp> HAVING <cond>."""
    ...
    initial_rows = draw(rows_for_table(table, min_size=3, max_size=15))

    agg_func = draw(st.sampled_from(["SUM", "COUNT"]))
    agg_expr = f"{agg_func}({agg_name})" if agg_func != "COUNT" else "COUNT(*)"

    # Generate a HAVING condition based on the aggregate
    having_op = draw(st.sampled_from([">", ">="]))
    having_threshold = draw(st.integers(min_value=1, max_value=5))

    if agg_func == "COUNT":
        having_clause = f"HAVING COUNT(*) {having_op} {having_threshold}"
    else:
        having_clause = f"HAVING {agg_func}({agg_name}) {having_op} {having_threshold}"

    view_sql = (
        f"SELECT {group_name}, {agg_expr} AS agg_val "
        f"FROM {tname} GROUP BY {group_name} {having_clause}"
    )
